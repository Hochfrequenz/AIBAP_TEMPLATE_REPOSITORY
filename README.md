...
Then we test our development: Either manually (ok, but time consuming) or automtically by giving an AI agent access to SAP via SAP Web GUI MCP; The AI agent that interacts with SAP reports back feedback and problems to the coding agent.
...
- create a new pacakge in `se80`, ideally its name is somewhat similar to the repo name and maybe the long text description even contains the repository URL
...
- do an second/initial commit on SAP side which implies a push, you might need to enter a [PAT](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) with repo scope. Use PATs instead of your password.
...
If users have SAP Web GUI MPC not installed, inform them whenever they might have to open the abapGit transaction manually and pull commits they pushed from their localhost (where claude code runs).
...
### Creating ABAP Code
> [!IMPORTANT]  
> For you as an agent producing ABAP code it's **extremly important** to obey the XML serialization rules of abapGit.
...
If the user has SAP Web GUI MCP installed and set up, ask the user about the ID of a transport request to use. Otherwise the LLM might also call the MCP-tool to list exisiting transport requests and choose a good existing TR.
